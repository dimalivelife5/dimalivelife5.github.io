...
echo "<br><Br>Поставщики<br><br>";
  ...
  <form method="POST">
	...
	<tr>
		...
		<td>представитель: <br><input type=text  name="представитель"><br></td>
		<td>телефон: <br><input type=text  name="телефон"><br></td>
		</tr>
		<br><input type=submit value="Добавить" name="sub12"><br>
	</form>
<?php	
	if (isset($_POST['sub12']))
		{
			...
			if (empty($_POST['представитель']))
			{
				echo "\n\nПоле представитель пустое !";
			}
			
			else if (empty($_POST['телефон']))
			{
				echo "\n\nПоле телефон пустое!";
			}
			else{
						
				$sql = "SELECT Код_поставщика FROM postavchiki";
				$result = $conn->query($sql);

				//находим последний id
				while ($row = $result->fetch_assoc()) 
				{
					$Код_поставщика = $row['Код_поставщика'];
				}
				$Код_поставщика = $Код_поставщика+1;
				$название = $_POST['название'];
				$представитель = $_POST['представитель'];
				$телефон = $_POST['телефон'];
				
										
				//INSERT

				$sql = "INSERT INTO postavchiki (Код_поставщика, название, представитель, телефон) VALUES('".$Код_поставщика."', '".$название."', '".$представитель."','".$телефон."')";
					
				$result = $conn->query($sql);
				$sql = "SELECT * FROM postavchiki";
				$result = $conn->query($sql);
				echo "<br><br>1. Добавить новую запись.<br>";
				echo "<br>Запрос:".$sql." values('".$Код_поставщика."', '".$название."', '".$представитель."','".$телефон."')";
				if($result->num_rows > 0) 
				{
					...
					while($row = $result->fetch_assoc()) 
					{
						echo "<tr><td>".$row["Код_поставщика"]."</td><td>".$row["название"]."</td><td>".$row["представитель"]."</td><td>".$row["телефон"]."</td></tr>";
					}
					echo "</table>";
				}else {echo "0 results";}
			}
