...
echo "<br><Br>Заказано<br><br>";
  ...
  ?>
	...
	<tr>
		...
		<td>Код_заказа: <br><input type=text  name="Код_заказа"><br></td>
		<td>Код_препарата: <br><input type=text  name="Код_препарата"><br></td>
		<td>цена_реализации: <br><input type=text  name="цена_реализации"><br></td>
		<td>колличество: <br><input type=text  name="колличество"><br></td>
		</tr>
...
<?php	
	if (isset($_POST['sub10']))
		{
			if (empty($_POST['Код_препарата']))
			{
				echo "\n\nПоле Код_препарата пустое !";
			}
			if (empty($_POST['цена_реализации']))
			{
				echo "\n\nПоле цена_реализации пустое !";
			}
			
			else if (empty($_POST['колличество']))
			{
				echo "\n\nПоле колличество пустое!";
			}
			else{
						
				
				$Код_заказа = $_POST['Код_заказа'];
				$Код_препарата = $_POST['Код_препарата'];
				$цена_реализации = $_POST['цена_реализации'];
				$колличество = $_POST['колличество'];
				
										
				//INSERT

				$sql = "INSERT INTO zakazano (Код_заказа, Код_препарата, цена_реализации, колличество) VALUES('".$Код_заказа."', '".$Код_препарата."', '".$цена_реализации."', '".$колличество."')";
					
				$result = $conn->query($sql);
				$sql = "SELECT * FROM zakazano";
				$result = $conn->query($sql);
				echo "<br><br>1. Добавить новую запись.<br>";
				echo "<br>Запрос:".$sql." values('".$Код_заказа."', '".$Код_препарата."', '".$цена_реализации."', '".$колличество."')";
				if($result->num_rows > 0) 
				{
					...
					while($row = $result->fetch_assoc()) 
					{
						echo "<tr><td>".$row["Код_заказа"]."</td><td>".$row["Код_препарата"]."</td><td>".$row["цена_реализации"]."</td><td>".$row["колличество"]."</td></tr>";
					}
					echo "</table>";
				}else {echo "0 results";}
			}
