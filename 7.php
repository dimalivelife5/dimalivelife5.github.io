...
<?php
	...
	if(isset($_GET['del1']))
	{
		$Код_поставщика=$_GET['del1'];
		$sql = "DELETE FROM postavchiki WHERE Код_поставщика=".$Код_поставщика."";
		$result = $conn->query($sql);
		echo '<meta http-equiv="refresh" content="0; url=http://localhost/progect/7.php">';
	}
	else {
		...
		if($result->num_rows > 0) {
			echo "<br><b>Table postavchiki</b><br><br>";
			echo "<table border=2>";
			echo "<tr><td>Код_поставщика</td>"."<td>название</td>"."<td>представитель</td>"."<td>телефон</td>"."<td>Удаление</td>"."<td>Редактирование</td></tr>";
			while($row = $result->fetch_assoc())
			{
				echo "<tr><td>".$row["Код_поставщика"]."</td><td>".$row["название"]."</td><td>".$row["представитель"]."</td><td>".$row["телефон"]."</td>
				<td><a href = http://localhost/progect/7.php?del1=".$row["Код_поставщика"].">Удалить</td><td><a href = http://localhost/progect/7.php?up=".$row["Код_поставщика"]."&название=".$row["название"]."&представитель=".$row["представитель"]."&телефон=".$row["телефон"].">Изменить</a></td></tr>";
			}
			echo "</table>";
		} else {echo "0 results";} 
		echo "<br><br><br>";
...
<?php 
			...
			if ($_GET[up] != 0) 
			{
				echo"<form>
					...
					<tr><td>представитель</td><td><input type='text' name='text6' value = '$_GET[представитель]'></td></tr>
					<tr><td>телефон</td><td><input type='text' name='text8' value = '$_GET[телефон]'></td></tr>
			...
			{
				$sql = "UPDATE postavchiki SET название = '".$название."', представитель = '".$представитель."', телефон = '".$телефон."' WHERE Код_поставщика =".$Код_поставщика;
	$result = $conn->query($sql);
	echo '<meta http-equiv="refresh" content="0; url=http://localhost/progect/7.php">';
			}
		?>
