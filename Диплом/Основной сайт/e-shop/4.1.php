...
echo "<br><Br>Клиенты<br><br>";
  ...
  ?>
	...
	<tr>
		...
		<br><input type=submit value="Добавить" name="sub9"><br>
	</form>
<?php	
	if (isset($_POST['sub9']))
		{
			...
			else if (empty($_POST['телефон']))
			{
				...
				$sql = "SELECT Код_клиента FROM klient";
				$result = $conn->query($sql);

				//находим последний id
				while ($row = $result->fetch_assoc()) 
				{
					$Код_клиента = $row['Код_клиента'];
				}
				$Код_клиента = $Код_клиента+1;
				...
				$sql = "INSERT INTO klient (Код_клиента, название, представитель, телефон) VALUES('".$Код_клиента."', '".$название."', '".$представитель."','".$телефон."')";
					
				$result = $conn->query($sql);
				$sql = "SELECT * FROM klient";
				$result = $conn->query($sql);
				echo "<br><br>1. Добавить новую запись.<br>";
				echo "<br>Запрос:".$sql." values('".$Код_клиента."', '".$название."', '".$представитель."','".$телефон."')";
				if($result->num_rows > 0) 
				{
					...
					while($row = $result->fetch_assoc()) 
					{
						echo "<tr><td>".$row["Код_клиента"]."</td><td>".$row["название"]."</td><td>".$row["представитель"]."</td><td>".$row["телефон"]."</td></tr>";
					}
					echo "</table>";
				}else {echo "0 results";}
			}
