...
<?php
	...
	if(isset($_GET['del1']))
	{
		$Код_препарата=$_GET['del1'];
		$sql = "DELETE FROM preparati WHERE Код_препарата=".$Код_препарата."";
		$result = $conn->query($sql);
		echo '<meta http-equiv="refresh" content="0; url=http://localhost/progect/8.php">';
	}
	else {
		...
		if($result->num_rows > 0) {
			...
			echo "<table border=2>";
			echo "<tr><td>Код_препарата</td>"."<td>название</td>"."<td>код_назначения</td>"."<td>код_поставщика</td>"."<td>цена_закупочная</td>"."<td>цена_реализация</td>"."<td>Удаление</td>"."<td>Редактирование</td></tr>";
			while($row = $result->fetch_assoc())
			{
				echo "<tr><td>".$row["Код_препарата"]."</td><td>".$row["название"]."</td><td>".$row["код_назначения"]."</td><td>".$row["код_поставщика"]."</td><td>".$row["цена_закупочная"]."</td><td>".$row["цена_реализация"]."</td>
				<td><a href = http://localhost/progect/8.php?del1=".$row["Код_препарата"].">Удалить</td><td><a href = http://localhost/progect/8.php?up=".$row["Код_препарата"]."&название=".$row["название"]."&код_назначения=".$row["код_назначения"]."&код_поставщика=".$row["код_поставщика"]."&цена_закупочная=".$row["цена_закупочная"]."&цена_реализация=".$row["цена_реализация"].">Изменить</a></td></tr>";
			}
			echo "</table>";
		} else {echo "0 results";} 
		echo "<br><br><br>";
...
<?php 
			$Код_препарата=$_GET[up];
			
			if ($_GET[up] != 0) 
			{
				echo"<form>
					<table>
					<tr><td>название</td><td><input type='text' name='text5' value = '$_GET[название]'></td></tr>
					<tr><td>код_назначения</td><td><input type='text' name='text6' value = '$_GET[код_назначения]'></td></tr>
					<tr><td>код_поставщика</td><td><input type='text' name='text8' value = '$_GET[код_поставщика]'></td></tr>
					<tr><td>цена_закупочная</td><td><input type='text' name='text9' value = '$_GET[цена_закупочная]'></td></tr>
					<tr><td>цена_реализация</td><td><input type='text' name='text10' value = '$_GET[цена_реализация]'></td></tr>
			...
			$название=$_GET[text5];
			$код_назначения=$_GET[text6];
			$код_поставщика=$_GET[text8];
			$цена_закупочная=$_GET[text9];
			$цена_реализация=$_GET[text10];
			...
			{
				$sql = "UPDATE preparati SET название = '".$название."', код_назначения = '".$код_назначения."', код_поставщика = '".$код_поставщика."', цена_закупочная = '".$цена_закупочная."', цена_реализация = '".$цена_реализация."' WHERE Код_препарата =".$Код_препарата;
	$result = $conn->query($sql);
	echo '<meta http-equiv="refresh" content="0; url=http://localhost/progect/8.php">';
			}
		?>
