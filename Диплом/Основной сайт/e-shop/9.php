...
<?php
	...
	if(isset($_GET['del1']))
	{
		$Код_заказа=$_GET['del1'];
		$sql = "DELETE FROM zakazano WHERE Код_заказа=".$Код_заказа."";
		$result = $conn->query($sql);
		echo '<meta http-equiv="refresh" content="0; url=http://localhost/progect/9.php">';
	}
	else {
		...
		if($result->num_rows > 0) {
			echo "<br><b>Table zakazano</b><br><br>";
			echo "<table border=2>";
			echo "<tr><td>Код_заказа</td>"."<td>Код_препарата</td>"."<td>цена_реализации</td>"."<td>колличество</td>"."<td>удаление</td>"."<td>Редактирование</td></tr>";
			while($row = $result->fetch_assoc())
			{
				echo "<tr><td>".$row["Код_заказа"]."</td><td>".$row["Код_препарата"]."</td><td>".$row["цена_реализации"]."</td><td>".$row["колличество"]."</td>
				<td><a href = http://localhost/progect/9.php?del1=".$row["Код_заказа"].">Удалить</td><td><a href = http://localhost/progect/9.php?up=".$row["Код_заказа"]."&Код_препарата=".$row["Код_препарата"]."&цена_реализации=".$row["цена_реализации"]."&колличество=".$row["колличество"].">Изменить</a></td></tr>";
			}
			echo "</table>";
		} else {echo "0 results";} 
		echo "<br><br><br>";
...
<?php 
			...
			if ($_GET[up] != 0) 
			{
				echo"<form>
					<table>
					<tr><td>Код_препарата</td><td><input type='text' name='text5' value = '$_GET[Код_препарата]'></td></tr>
					<tr><td>цена_реализации</td><td><input type='text' name='text6' value = '$_GET[цена_реализации]'></td></tr>
					<tr><td>колличество</td><td><input type='text' name='text8' value = '$_GET[колличество]'></td></tr>
					<tr><td><input type='submit' name='submit2' value='Изменить' /></td></tr>	
					</table>
					<input type='hidden' name='up' value = $_GET[up]>
					</form>";					
			}
			$Код_препарата=$_GET[text5];
			$цена_реализации=$_GET[text6];
			$колличество=$_GET[text8];
			...
			{
				$sql = "UPDATE zakazano SET Код_препарата = '".$Код_препарата."', цена_реализации = '".$цена_реализации."', колличество = '".$колличество."' WHERE Код_заказа =".$Код_заказа;
	$result = $conn->query($sql);
	echo '<meta http-equiv="refresh" content="0; url=http://localhost/progect/9.php">';
			}
		?>
