...
<?php
	...
	if(isset($_GET['del1']))
	{
		$Код_сотрудника=$_GET['del1'];
		$sql = "DELETE FROM cotrydniki WHERE Код_сотрудника=".$Код_сотрудника."";
		$result = $conn->query($sql);
		echo '<meta http-equiv="refresh" content="0; url=http://localhost/progect/4.php">';
	}
	else {
		$sql = "SELECT * FROM cotrydniki";
		$result = $conn->query($sql);

		if($result->num_rows > 0) {
			echo "<br><b>Table cotrydniki</b><br><br>";
			echo "<table border=2>";
			echo "<tr><td>Код_сотрудника</td>"."<td>Фамилия</td>"."<td>Имя</td>"."<td>Отчество_27</td>"."<td>дата_рождения</td>"."<td>должность</td>"."<td>Удаление</td>"."<td>Редактирование</td></tr>";
			while($row = $result->fetch_assoc())
			{
				echo "<tr><td>".$row["Код_сотрудника"]."</td><td>".$row["Фамилия"]."</td><td>".$row["Имя"]."</td><td>".$row["Отчество_27"]."</td><td>".$row["дата_рождения"]."</td><td>".$row["должность"]."</td>
				<td><a href = http://localhost/progect/4.php?del1=".$row["Код_сотрудника"].">Удалить</td><td><a href = http://localhost/progect/4.php?up=".$row["Код_сотрудника"]."&Фамилия=".$row["Фамилия"]."&Имя=".$row["Имя"]."&Отчество_27=".$row["Отчество_27"]."&дата_рождения=".$row["дата_рождения"]."&должность=".$row["должность"].">Изменить</a></td></tr>";
			}
			echo "</table>";
		} else {echo "0 results";} 
		echo "<br><br><br>";
...
<?php 
			$Код_сотрудника=$_GET[up];
			
			if ($_GET[up] != 0) 
			{
				echo"<form>
					<table>
					<tr><td>Фамилия</td><td><input type='text' name='text5' value = '$_GET[Фамилия]'></td></tr>
					<tr><td>Имя</td><td><input type='text' name='text6' value = '$_GET[Имя]'></td></tr>
					<tr><td>Отчество_27</td><td><input type='text' name='text8' value = '$_GET[Отчество_27]'></td></tr>
					<tr><td>дата_рождения</td><td><input type='text' name='text9' value = '$_GET[дата_рождения]'></td></tr>
					<tr><td>должность</td><td><input type='text' name='text10' value = '$_GET[должность]'></td></tr>
			...
			$Фамилия=$_GET[text5];
			$Имя=$_GET[text6];
			$Отчество_27=$_GET[text8];
			$дата_рождения=$_GET[text9];
			$должность=$_GET[text10];
			...
			{
				$sql = "UPDATE cotrydniki SET Фамилия = '".$Фамилия."', Имя = '".$Имя."', Отчество_27 = '".$Отчество_27."', дата_рождения = '".$дата_рождения."', должность = '".$должность."' WHERE Код_сотрудника =".$Код_сотрудника;
	$result = $conn->query($sql);
	echo '<meta http-equiv="refresh" content="0; url=http://localhost/progect/4.php">';
			}
		?>
