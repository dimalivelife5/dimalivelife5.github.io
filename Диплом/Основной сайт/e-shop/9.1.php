...
echo "<br><Br>Заказано<br><br>";
  ...
  <form method="POST">
	...
	<tr>
		...
		<td>Код_клиента: <br><input type=text  name="Код_клиента"><br></td>
		<td>Код_сотрудника: <br><input type=text  name="Код_сотрудника"><br></td>
		<td>Дата: <br><input type=text  name="Дата"><br></td>
		</tr>
		<br><input type=submit value="Добавить" name="sub14"><br>
	</form>
<?php	
	if (isset($_POST['sub14']))
		{
			if (empty($_POST['Код_клиента']))
			{
				echo "\n\nПоле Код_клиента пустое !";
			}
			if (empty($_POST['Код_сотрудника']))
			{
				echo "\n\nПоле Код_сотрудника пустое !";
			}
			
			else if (empty($_POST['Дата']))
			{
				echo "\n\nПоле Дата пустое!";
			}
			else{
						
				$sql = "SELECT Код_заказа FROM zakazy";
				$result = $conn->query($sql);

				//находим последний id
				while ($row = $result->fetch_assoc()) 
				{
					$Код_заказа = $row['Код_заказа'];
				}
				$Код_заказа = $Код_заказа+1;
				$Код_клиента = $_POST['Код_клиента'];
				$Код_сотрудника = $_POST['Код_сотрудника'];
				$Дата = $_POST['Дата'];
				
										
				//INSERT

				$sql = "INSERT INTO zakazy (Код_заказа, Код_клиента, Код_сотрудника, Дата) VALUES('".$Код_заказа."', '".$Код_клиента."', '".$Код_сотрудника."', '".$Дата."')";
					
				$result = $conn->query($sql);
				$sql = "SELECT * FROM zakazy";
				$result = $conn->query($sql);
				echo "<br><br>1. Добавить новую запись.<br>";
				echo "<br>Запрос:".$sql." values('".$Код_заказа."', '".$Код_клиента."', '".$Код_сотрудника."', '".$Дата."')";
				if($result->num_rows > 0) 
				{
					...
					while($row = $result->fetch_assoc()) 
					{
						echo "<tr><td>".$row["Код_заказа"]."</td><td>".$row["Код_клиента"]."</td><td>".$row["Код_сотрудника"]."</td><td>".$row["Дата"]."</td></tr>";
					}
					echo "</table>";
				}else {echo "0 results";}
			}
