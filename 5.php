...
<?php
	...
	if(isset($_GET['del1']))
	{
		$Код_клиента=$_GET['del1'];
		$sql = "DELETE FROM klient WHERE Код_клиента=".$Код_клиента."";
		$result = $conn->query($sql);
		echo '<meta http-equiv="refresh" content="0; url=http://localhost/progect/5.php">';
	}
	else {
		...
		if($result->num_rows > 0) {
			echo "<br><b>Table klient</b><br><br>";
			echo "<table border=2>";
			echo "<tr><td>Код_клиента</td>"."<td>название</td>"."<td>представитель</td>"."<td>телефон</td>"."<td>удаление</td>"."<td>Редактирование</td></tr>";
			while($row = $result->fetch_assoc())
			{
				echo "<tr><td>".$row["Код_клиента"]."</td><td>".$row["название"]."</td><td>".$row["представитель"]."</td><td>".$row["телефон"]."</td>
				<td><a href = http://localhost/progect/5.php?del1=".$row["Код_клиента"].">Удалить</td><td><a href = http://localhost/progect/5.php?up=".$row["Код_клиента"]."&название=".$row["название"]."&представитель=".$row["представитель"]."&телефон=".$row["телефон"].">Изменить</a></td></tr>";
			}
			echo "</table>";
		} else {echo "0 results";} 
		echo "<br><br><br>";
...
<?php 
			...
			{
				$sql = "UPDATE klient SET название = '".$название."', представитель = '".$представитель."', телефон = '".$телефон."' WHERE Код_клиента =".$Код_клиента;
	$result = $conn->query($sql);
	echo '<meta http-equiv="refresh" content="0; url=http://localhost/progect/5.php">';
			}
		?>
