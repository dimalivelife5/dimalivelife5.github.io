...
<?php
	...
	if(isset($_GET['del1']))
	{
		...
		$sql = "DELETE FROM zakazy WHERE Код_заказа=".$Код_заказа."";
		$result = $conn->query($sql);
		echo '<meta http-equiv="refresh" content="0; url=http://localhost/progect/10.php">';
	}
	else {
		...
		if($result->num_rows > 0) {
			echo "<br><b>Table zakazy</b><br><br>";
			echo "<table border=2>";
			echo "<tr><td>Код_заказа</td>"."<td>Код_клиента</td>"."<td>Код_сотрудника</td>"."<td>Дата</td>"."<td>удаление</td>"."<td>Редактирование</td></tr>";
			while($row = $result->fetch_assoc())
			{
				echo "<tr><td>".$row["Код_заказа"]."</td><td>".$row["Код_клиента"]."</td><td>".$row["Код_сотрудника"]."</td><td>".$row["Дата"]."</td>
				<td><a href = http://localhost/progect/10.php?del1=".$row["Код_заказа"].">Удалить</td><td><a href = http://localhost/progect/10.php?up=".$row["Код_заказа"]."&Код_клиента=".$row["Код_клиента"]."&Код_сотрудника=".$row["Код_сотрудника"]."&Дата=".$row["Дата"].">Изменить</a></td></tr>";
			}
			echo "</table>";
		} else {echo "0 results";} 
		echo "<br><br><br>";
...
<?php 
			...
			if ($_GET[up] != 0) 
			{
				echo"<form>
					<table>
					<tr><td>Код_клиента</td><td><input type='text' name='text5' value = '$_GET[Код_клиента]'></td></tr>
					<tr><td>Код_сотрудника</td><td><input type='text' name='text6' value = '$_GET[Код_сотрудника]'></td></tr>
					<tr><td>Дата</td><td><input type='text' name='text8' value = '$_GET[Дата]'></td></tr>
					<tr><td><input type='submit' name='submit2' value='Изменить' /></td></tr>	
					</table>
					<input type='hidden' name='up' value = $_GET[up]>
					</form>";					
			}
			$Код_клиента=$_GET[text5];
			$Код_сотрудника=$_GET[text6];
			$Дата=$_GET[text8];
			...
			{
				$sql = "UPDATE zakazy SET Код_клиента = '".$Код_клиента."', Код_сотрудника = '".$Код_сотрудника."', Дата = '".$Дата."' WHERE Код_заказа =".$Код_заказа;
	$result = $conn->query($sql);
	echo '<meta http-equiv="refresh" content="0; url=http://localhost/progect/10.php">';
			}
		?>
