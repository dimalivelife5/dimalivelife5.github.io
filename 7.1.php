...
<?php
  ...
  $conn = new mysqli($servername_27, $username_27, $password_27, $dbname_27);
	...
	echo "<br><Br>Препараты<br><br>";
	

	$sql = "SELECT * FROM preparati";
	$result = $conn->query($sql);
	if($result->num_rows > 0) {
		echo "<table border=2>";
	echo "<tr><td>Код_препарата</td>"."<td>название</td>"."<td>	Код_назначения</td>"."<td>Код_препарата</td>"."<td>цена_закупочная</td>"."<td>цена_реализация</td></tr>";
		while($row = $result->fetch_assoc()) {
			echo "<tr>
			<td>".$row["Код_препарата"]."</td>
			<td>".$row["название"]."</td>
			<td>".$row["код_назначения"]."</td>
				<td>".$row["Код_препарата"]."</td>
				<td>".$row["цена_закупочная"]."</td>
				<td>".$row["цена_реализация"]."</td>
				</tr>";
		}
		echo "</table>";
	}else {
  ...
  ?>
   <form method="POST">
	...
	<tr>
		...
		<td>название: <br><input type=text  name="название"><br></td>
		<td>код_назначения: <br><input type=text  name="код_назначения"><br></td>
		<td>код_поставщика: <br><input type=text  name="код_поставщика"><br></td>
		<td>цена_закупочная: <br><input type=text  name="цена_закупочная"><br></td>
		<td>цена_реализация: <br><input type=text  name="цена_реализация"><br></td>
		</tr>
		<br><input type=submit value="Добавить" name="sub13"><br>
	</form>
<?php	
	if (isset($_POST['sub13']))
		{
			if (empty($_POST['название']))
			{
			echo "\n\nПоле название пустое !";}
				
			if (empty($_POST['код_назначения']))
			{
			echo "\n\nПоле код_назначения пустое !";}	
				
			if (empty($_POST['код_поставщика']))
			{
				echo "\n\nПоле код_поставщика пустое !";	
				
				
			}
			if (empty($_POST['цена_закупочная']))
			{
				echo "\n\nПоле цена_закупочная пустое !";
			}
			
			else if (empty($_POST['цена_реализация']))
			{
				echo "\n\nПоле цена_реализация пустое!";
			}
			else{
						
				$sql = "SELECT Код_препарата FROM preparati";
				$result = $conn->query($sql);

				//находим последний id
				while ($row = $result->fetch_assoc()) 
				{
					$Код_препарата = $row['Код_препарата'];
				}
				$Код_препарата = $Код_препарата+1;
				$название = $_POST['название'];
				$код_назначения = $_POST['код_назначения'];
				$код_поставщика = $_POST['код_поставщика'];
				$цена_закупочная = $_POST['цена_закупочная'];
				$цена_реализация = $_POST['цена_реализация'];
				
										
				//INSERT

				$sql = "INSERT INTO preparati (Код_препарата, название, код_назначения, код_поставщика, цена_закупочная, цена_реализация) VALUES('".$Код_препарата."', '".$название."', '".$код_назначения."', '".$код_поставщика."', '".$цена_закупочная."', '".$цена_реализация."')";
					
				$result = $conn->query($sql);
				$sql = "SELECT * FROM preparati";
				$result = $conn->query($sql);
				echo "<br><br>1. Добавить новую запись.<br>";
				echo "<br>Запрос:".$sql." values('".$Код_препарата."', '".$название."', '".$код_назначения."', '".$код_поставщика."', '".$цена_закупочная."','".$цена_реализация."')";
				if($result->num_rows > 0) 
				{
					echo "<br><b>Table preparati</b><br><br>";
					echo "<table border=2>";
					echo "<tr><td>Код_препарата </td>"."<td>название</td>"."<td>код_назначения</td>"."<td>код_поставщика</td>"."<td>цена_закупочная</td>"."<td>цена_реализация</td>"."</tr>";
					while($row = $result->fetch_assoc()) 
					{
						echo "<tr><td>".$row["Код_препарата"]."</td><td>".$row["название"]."</td><td>".$row["код_назначения"]."</td><td>".$row["код_поставщика"]."</td><td>".$row["цена_закупочная"]."</td><td>".$row["цена_реализация"]."</td></tr>";
					}
					echo "</table>";
				}else {echo "0 results";}
			}
